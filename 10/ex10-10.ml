...
let global_ekikan_list = [
  ...
  {kiten="東高円寺"; shuten="��高円寺"; keiyu="丸ノ内線"; kyori=0.9; jikan=1};
  {kiten="新高円寺"; shuten="南阿佐ヶ谷"; keiyu="丸ノ内線"; kyori=1.2; jikan=2};
  {kiten="南阿��ヶ谷"; shuten="荻窪"; keiyu="丸ノ内線"; kyori=1.5; jikan=2};
...
let rec get_ekikan_kyori n1 n2 list = match list with
    [] -> infinity
  | {kiten = k; shuten = s; kyori = d} :: rest ->
    if (n1 = k && n2 = s) || (n1 = s && n2 = k)
    then d
    else get_ekikan_kyori n1 n2 rest ;;

let rec kyori_wo_hyoji n1 n2 =
  let kanji1 = romaji_to_kanji n1 global_ekimei_list in
  if kanji1 = ""
  then n1 ^ "と言う駅は存在しません。"
  else
    let kanji2 = romaji_to_kanji n2 global_ekimei_list in
    if kanji2 = ""
    then n2 ^ "と言う駅は存在しません。"
    else let kyori = get_ekikan_kyori kanji1 kanji2 global_ekikan_list in
      if kyori = infinity
      then "AとBは繋がっていません。"
      else string_of_float kyori
;;
